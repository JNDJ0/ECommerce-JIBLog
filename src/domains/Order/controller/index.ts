...
router.patch('/:code/status', verifyJWT, checkUserRole(['user', 'owner', 'deliverer', 'admin']), async (req: Request, res: Response, next: NextFunction) => {
	try {
		const { status } = req.body;
		await OrderService.updateStatus(req.params.code, status, req.user.role, +req.user.id);
		res.json(`Status atualizado para '${status}' com sucesso.`);
	} catch (error) {
		next(error);
	}
});
